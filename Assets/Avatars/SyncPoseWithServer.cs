...
namespace Outdated
{
    ...
    public class SyncPoseWithServer : MonoBehaviour
    {
        ...
        void ApplySkeletonToAvatar(SkeletonData skeletonData)
        {
            ...
            // avatar.transform.localPosition = characterPos;

            KinectJoint[] analysedJoints = {KinectJoint.PELVIS};

            foreach (JointData joint in skeletonData.joints)
            {
                if (!analysedJoints.Contains(joint.joint))
                {
                    // continue;
                }

                if (joint.joint == KinectJoint.PELVIS)
                {
                    Debug.Log($"DEBUG: Raw Kinect Pelvis Orientation: {joint.orientation}");
                    var converted = ConvertFromKinectQuaternion(joint.orientation);
                    Debug.Log($"DEBUG: Converted Pelvis Orientation: {converted}");
                    Debug.Log($"DEBUG: Converted Pelvis Euler Angles: {converted.eulerAngles}");
                }
